...
def test_register_artist_verified_when_score_reaches_70(
    ...
    with patch("contracts.ProvenanceRegistry.ENABLE_ACOUSTID", True), \
         patch("contracts.ProvenanceRegistry.ENABLE_SPOTIFY", True), \
         patch("contracts.ProvenanceRegistry._acoustid_lookup", return_value=(True, "mbid-1")), \
...
def test_register_artist_not_verified_when_no_signals(
    ...
    with patch("contracts.ProvenanceRegistry._acoustid_lookup", return_value=(False, "")), \
         ...
         patch("contracts.ProvenanceRegistry._instagram_fetch", return_value=("", "", "")), \
...
def test_get_artist_unregistered(contract, artist_wallet):
    assert contract.get_artist(artist_wallet) == {"verified": False, "revoked": False}
...
def test_disabled_collectors_are_not_called(contract, artist_wallet, audio_hash, source_urls):
    from contracts.ProvenanceRegistry import ENABLE_SPOTIFY, ENABLE_ACOUSTID
    assert ENABLE_SPOTIFY is False and ENABLE_ACOUSTID is False
