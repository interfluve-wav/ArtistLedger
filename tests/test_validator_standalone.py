...
def has_tier1_signal(leader_dict: dict) -> bool:
    """Returns True iff at least one real-world presence signal is present.

    Mirrors the tier-1 floor. v0.6.1 (audit F2): verification_match_count
    no longer counts — claimed sources are submitter-chosen and can't be
    the floor against phantom evidence.
    v0.7.2: a confirmed ownership proof (ALVERIFY token on the artist's
    own profile) also satisfies the floor — it is stronger evidence than
    a scraped catalog ID.
    """
    return (
...
# ─── Audit F1/F2 regression (2026-09-15) ─────────────────────────────────

def test_tier1_floor_rejects_claimed_only_evidence():
    """F2: verification_match_count=2 with NO catalog signal must NOT pass
    the floor — claimed sources are submitter-chosen."""
    ev = empty_evidence()
    ev["verification_source_1"] = "soundcloud_url"
    ev["verification_handle_1"] = "attacker-page"
    ev["verification_source_2"] = "instagram"
    ev["verification_handle_2"] = "attacker-page"
    ev["verification_match_count"] = 2
    assert has_tier1_signal(ev) is False
    assert validator_accepts(ev, "Any Name") is False


def test_tier1_floor_still_accepts_catalog_signals():
    """The floor keeps working for genuine catalog evidence."""
    ev = empty_evidence()
    ev["spotify_artist_id"] = "abc123"
    assert has_tier1_signal(ev) is True
    ev2 = empty_evidence()
    ev2["acoustid_matched"] = True
    assert has_tier1_signal(ev2) is True
    ev3 = empty_evidence()
    ev3["apple_music_artist_id"] = "12345"
    assert has_tier1_signal(ev3) is True


def test_tier1_floor_accepts_ownership_proof():
    """v0.7.2: an ALVERIFY ownership proof satisfies the floor — a token
    physically on the artist's own profile is real-world presence, even
    with zero catalog signals (no Apple/Spotify/AcoustID). This is the
    exact case that made Interfluve hit MAJORITY_DISAGREE."""
    for key in ("ownership_proof_soundcloud", "ownership_proof_lastfm",
                "ownership_proof_bandcamp", "ownership_proof_youtube"):
        ev = empty_evidence()
        ev[key] = True
        assert has_tier1_signal(ev) is True, key
        assert validator_accepts(ev, "Interfluve") is True, key
    # and the converse stays: no catalog, no proof -> still rejected
    ev = empty_evidence()
    ev["bandcamp_handle"] = "ghost"
    assert has_tier1_signal(ev) is False
    assert validator_accepts(ev, "Ghost") is False
...
def test_soundcloud_check_falsy_on_failure():
    """F1: _soundcloud_check must return an empty handle when the fetch
    fails (previously returned the input handle — truthy — for any input)."""
    from unittest.mock import patch
    from contracts.ProvenanceRegistry import _soundcloud_check
    with patch("contracts.ProvenanceRegistry._http_get", side_effect=Exception("down")):
        h, followers, verified = _soundcloud_check("whatever-handle")
    assert h == "" and followers == 0 and verified is False


def test_verify_claimed_source_requires_name_bind():
    """F1: a claimed source whose page exists but whose display name does
    NOT overlap the claimed artist must not count as a match."""
    from unittest.mock import patch
    from contracts.ProvenanceRegistry import _verify_claimed_source
    with patch("contracts.ProvenanceRegistry._soundcloud_check", return_value=("victim-handle", 0, False)), \
         patch("contracts.ProvenanceRegistry._soundcloud_display_name", return_value="Completely Unrelated Page"):
        assert _verify_claimed_source("soundcloud_url", "victim-handle", "Famous DJ", "", "") is False
    with patch("contracts.ProvenanceRegistry._soundcloud_check", return_value=("famous-dj", 0, False)), \
         patch("contracts.ProvenanceRegistry._soundcloud_display_name", return_value="Famous DJ Official"):
        assert _verify_claimed_source("soundcloud_url", "famous-dj", "Famous DJ", "", "") is True


# ─── Username-vs-artist-name tolerance (2026-09-15) ──────────────────────

def test_overlap_handles_punctuation_and_compound_usernames():
    from contracts.ProvenanceRegistry import _name_token_overlap
    # display name with punctuation
    assert _name_token_overlap("Fred again..", "Fred Again") >= 0.5
    # compound username containing the full artist name
    assert _name_token_overlap("aphextwinofficial", "Aphex Twin") >= 0.5
    # hyphenated
    assert _name_token_overlap(" Boards-of-Canada ", "Boards of Canada") >= 0.5
    # short names don't trigger containment (4-char minimum)
    assert _name_token_overlap("miafanpage", "M.I.A.") < 0.5
    # unrelated names stay unrelated
    assert _name_token_overlap("user823745", "Famous DJ") < 0.5


def test_sc_claim_binds_via_bio_when_username_differs():
    """Artist's SC username is 'user823745' but bio says 'official page of
    Famous DJ' — the claim must match via the bio path."""
    from unittest.mock import patch
    from contracts.ProvenanceRegistry import _verify_claimed_source
    with patch("contracts.ProvenanceRegistry._soundcloud_check", return_value=("user823745", 0, False)), \
         patch("contracts.ProvenanceRegistry._soundcloud_display_name", return_value="user823745"), \
         patch("contracts.ProvenanceRegistry._soundcloud_bio", return_value="The official page of Famous DJ. Bookings: x@y.com"):
        assert _verify_claimed_source("soundcloud_url", "user823745", "Famous DJ", "", "") is True


def test_sc_claim_still_rejects_unrelated_page():
    """Username AND bio both unrelated -> no match (the F1 attack stays dead)."""
    from unittest.mock import patch
    from contracts.ProvenanceRegistry import _verify_claimed_source
    with patch("contracts.ProvenanceRegistry._soundcloud_check", return_value=("user823745", 0, False)), \
         patch("contracts.ProvenanceRegistry._soundcloud_display_name", return_value="user823745"), \
         patch("contracts.ProvenanceRegistry._soundcloud_bio", return_value="I post mixes sometimes"):
        assert _verify_claimed_source("soundcloud_url", "user823745", "Famous DJ", "", "") is False
