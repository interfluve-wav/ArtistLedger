...
class TestLastfmResolveArtist:
    ...
    def _stub_search_and_getinfo(self, search_name, getinfo_name):
        """Search returns `search_name` (with an MBID — the popularity-
        ranked stranger case), getInfo returns `getinfo_name`."""
        def fake_get(url):
            if "method=artist.search" in url:
                body = json.dumps(self._search_response(search_name, "9999-0000"))
            else:
                body = json.dumps({"artist": {"name": getinfo_name}})
    ...
    def test_wrong_artist_search_hit_falls_back_to_getinfo(self):
        """Search ranks by popularity, so the top hit can be a stranger
        with an MBID ('Interfluve' → 'Aliyah's Interlude'). The overlap
        guard rejects that hit; getInfo with the raw claimed name resolves
        the canonical name instead."""
        orig = self._stub_search_and_getinfo("Aliyah's Interlude", "Interfluve")
        try:
            assert prov._lastfm_resolve_artist("Interfluve", "key") == "Interfluve"
        finally:
            prov.gl.nondet.web.get = orig

    def test_getinfo_fallback_rejects_unrelated_canonical(self):
        """Defensive: the getInfo fallback still name-binds — a canonical
        name that doesn't overlap the claim is rejected, never trusted."""
        orig = self._stub_search_and_getinfo("Aliyah's Interlude", "Completely Different")
        try:
            assert prov._lastfm_resolve_artist("Interfluve", "key") == ""
        finally:
            prov.gl.nondet.web.get = orig

    def test_overlapping_search_hit_still_wins(self):
        """The normal path is unchanged: a correct MBID match that overlaps
        the claimed name resolves directly (no getInfo round-trip)."""
        def fake_get(url):
            if "method=artist.search" in url:
                body = json.dumps(self._search_response("Fred Again...", "1234-5678"))
            else:  # getInfo must NOT be called
                raise AssertionError("getInfo fallback hit on a direct match")
            return types.SimpleNamespace(body=body.encode())
