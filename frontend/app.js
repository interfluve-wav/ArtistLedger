...
// Deployed on studionet (2026-09-16, gasless): all features incl.
// ownership proofs + key pools + LLM identity guard + F1/F2 claim-binding
// fixes + reverify identity rotation (v0.7.2, floor accepts proofs).
// Verified: schema OK, 15 methods. See docs/OPERATIONS.md.
const CONTRACT = "0xB110dA64B1c14B65c078430fd6Bd0f9E79d83981";
...
// ── Wallet-flow hardening (v0.7.1) ─────────────────────────────────────────
// Every provider call gets a deadline so a hung popup becomes a readable
// error instead of an endless "waiting" state. Chain-switch failures are
// never swallowed: a wallet on the WRONG chain is the #1 reason a real-wallet
// tx "submits" but never finalizes (the $0 approval goes through, the
// broadcast bounces off chainId checks).
function withTimeout(promise, ms, label) {
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => reject(new Error((label || "Wallet request") + " timed out — check for a pending popup in " + (activeWalletName || "your wallet"))), ms);
    Promise.resolve(promise).then(v => { clearTimeout(t); resolve(v); }, e => { clearTimeout(t); reject(e); });
  });
}

async function assertStudionetChain(provider, walletName) {
  const hex = STUDIONET_CHAIN_ID_HEX;
  const chainId = async () => {
    const id = await withTimeout(provider.request({ method: "eth_chainId" }), 15000, "chainId check");
    return String(id).toLowerCase();
  };
  try {
    if (await chainId() === hex.toLowerCase()) return true;   // already on studionet
  } catch (e) { /* provider may not answer eth_chainId yet — try switch */ }
  try {
    await withTimeout(provider.request({
      method: "wallet_switchEthereumChain", params: [{ chainId: hex }],
    }), 25000, "network switch");
  } catch (switchErr) {
    const msg = String(switchErr?.message || "");
    if (switchErr?.code === 4902 || switchErr?.code === -32603 || /unrecognized|not found|not supported|already been added/i.test(msg)) {
      await withTimeout(provider.request({
        method: "wallet_addEthereumChain", params: [STUDIONET_CHAIN_PARAMS],
      }), 30000, "add network");
      await withTimeout(provider.request({
        method: "wallet_switchEthereumChain", params: [{ chainId: hex }],
      }), 25000, "network switch");
    } else {
      throw new Error("Could not switch " + walletName + " to studionet (61999): " + (switchErr?.message || switchErr));
    }
  }
  const after = await chainId();
  if (after !== hex.toLowerCase()) {
    throw new Error(walletName + " is on chain " + after + ", not studionet (61999). Switch networks manually, then connect again.");
  }
  logLine("on studionet (61999) ✓");
  return true;
}

// Pre-write guard: cheap chainId check; auto-switch only when needed.
async function ensureStudionetAtSubmit() {
  if (walletKind !== "real" || !activeProvider) return true;
  try {
    const id = await withTimeout(activeProvider.request({ method: "eth_chainId" }), 12000, "chainId check");
    if (String(id).toLowerCase() === STUDIONET_CHAIN_ID_HEX.toLowerCase()) return true;
  } catch (e) { /* fall through to the switch attempt */ }
  return assertStudionetChain(activeProvider, activeWalletName);
}

function safeConnect(fn) {
  fn().catch(err => {
...
async function connectWithProvider(provider, walletName) {
  ...
  // 2. Switch or add studionet network (chainId 61999). Failures here are
  //    FATAL and visible — continuing on the wrong chain makes the $0
  //    approval succeed but the broadcast bounce (the old silent hang).
  await assertStudionetChain(provider, walletName);
  ...
  try {
    ...
    // Non-fatal: json-rpc signing works without the snap; log it so the
    // user knows why nothing extra popped up.
    logLine("[warn] GenLayer snap unavailable (" + (snapErr?.message || snapErr) + ") — json-rpc fallback");
  }
...
// Quick-start chips: one tap fills name + verified sources (strict off).
(function buildQuickStart() {
  const grid = $("qsGrid");
  if (!grid) return;
  const setActive = chip => {
    grid.querySelectorAll(".qs-chip").forEach(c => {
      const on = c === chip;
      c.classList.toggle("on", on);
      c.setAttribute("aria-pressed", String(on));
    });
  };
  Object.keys(DEMO_ARTISTS).forEach(name => {
...
// v0.7.1: derivation inputs are NORMALIZED (wallet lowercased, name
// lowercased) so casing of the typed name can never change the token —
// "Interfluve" and "interfluve" must mint the same string.
function ownershipKeyParts() {
  const w = (typeof walletAddr === "string" && /^0x/i.test(walletAddr)) ? walletAddr.toLowerCase() : "anon";
  const n = ($("f-name").value.trim() || "").toLowerCase();
  return w + ":" + n;
}
function genOwnershipToken() {
  const abc = "ABCDEFGHJKMNPQRSTVWXYZ23456789";
  const src = ownershipKeyParts();
  ...
  let rnd = "";
  let v = h >>> 0;
  // v0.7.1: modulo the alphabet LENGTH, not 32 — the alphabet is 30 chars,
  // so %32 minted literal "undefined" into ~40% of tokens (indices 30/31)
  for (let i = 0; i < 8; i++) { rnd += abc[v % abc.length]; v = Math.floor(v / abc.length); }
...
// ── Identity recovery (v0.7.0): reverify — identity ≠ wallet ─────────────
// The registration token is PUBLIC (it sits in the artist's bio), so it can
// never be a recovery credential. The token below is derived from the NEW
// connected wallet + name (+ a ":rv" salt so it never equals the register
// token) — by construction it has never appeared on-chain, which is exactly
// the single-use requirement the contract enforces.
function genRvToken() {
  const abc = "ABCDEFGHJKMNPQRSTVWXYZ23456789";
  const w = (typeof walletAddr === "string" && /^0x/i.test(walletAddr)) ? walletAddr.toLowerCase() : "anon";
  const n = ($("rv-name").value.trim() || "").toLowerCase();
  const src = w + ":" + n + ":rv";
  let h = 2166136261 >>> 0;
  for (let i = 0; i < src.length; i++) {
    h ^= src.charCodeAt(i);
  ...
  let rnd = ""; let v = h >>> 0;
  for (let i = 0; i < 8; i++) { rnd += abc[v % abc.length]; v = Math.floor(v / abc.length); }
  $("rv-token").value = `ALVERIFY-${rnd}-${Math.floor(Date.now() / 86400000)}`;
}

function rvProofsDict() {
  const out = {};
  document.querySelectorAll("#rv-rows .own-row").forEach(row => {
    const plat = row.querySelector("select").value;
    const handle = row.querySelector("input").value.trim();
    if (plat && handle) out[plat] = handle;
  });
...
function addRvRow(plat = "", handle = "") {
  ...
  if (plat) sel.value = plat;
  const inp = document.createElement("input");
  inp.placeholder = (OWN_PLATFORMS.find(p => p.id === plat) || {}).ph || "handle";
  inp.value = handle || "";
  sel.onchange = () => {
    inp.placeholder = (OWN_PLATFORMS.find(p => p.id === sel.value) || {}).ph || "handle";
  };
  const rm = document.createElement("button");
...
$("rv-add").onclick = () => addRvRow();
$("rv-gen").onclick = () => { genRvToken(); logLine("fresh recovery token derived for " + shortAddr(walletAddr || "anon")); };
$("rv-copy").onclick = async () => {
  try { await navigator.clipboard.writeText($("rv-token").value); $("rv-copy").textContent = "Copied ✓"; setTimeout(() => { $("rv-copy").textContent = "Copy"; }, 1200); } catch (e) {}
};
$("rv-name").addEventListener("input", genRvToken);
$("rv-old").addEventListener("input", () => { $("rv-old").value = $("rv-old").value.trim(); });

$("rv-submit").onclick = async () => {
  const rvOut = $("rv-status");
  if (!walletAddr) { formError("Click \"Connect\" first.", rvOut); return; }
  const oldAddr = $("rv-old").value.trim();
  const name = $("rv-name").value.trim();
  const token = $("rv-token").value.trim();
  const proofs = rvProofsDict();
  if (!oldAddr) { formError("Old wallet address is required.", rvOut); return; }
  if (!name) { formError("Artist name is required.", rvOut); return; }
  if (!token) { formError("Recovery token is required — paste it into your profile first.", rvOut); return; }
  if (Object.keys(proofs).length === 0) { formError("Add at least one profile that carries the token.", rvOut); return; }
  $("rv-submit").disabled = true;
  $("rv-submit").textContent = "Recovering…";
  const out = $("rv-status");
  out.innerHTML = "<b>Submitting to the contract…</b>";
  try {
    const args = [oldAddr, name, token, proofs];
    const callOpts = { address: CONTRACT, functionName: "reverify", args, value: 0n, leaderOnly: false };
    if (walletKind === "real") {
      callOpts.account = { type: "json-rpc", address: walletAddr };
      logLine("[rv] Waiting for the wallet to sign (check the popup)…");
    }
    const tx = await writeClient.writeContract(callOpts);
    out.innerHTML = "<b>Submitted " + tx.slice(0, 14) + "… — waiting for consensus (validators re-check the proof)</b>";
    logLine("[rv] Submitted tx " + tx.slice(0, 14) + "…");
    const receipt = await writeClient.waitForTransactionReceipt({ hash: tx, status: "FINALIZED", retries: 60, interval: 4000 });
    let msg = receipt.status_name + " / " + receipt.result_name;
    for (const lr of receipt.consensus_data.leader_receipt || []) {
      if (lr.execution_result === "ERROR") continue;
      if (lr.result && lr.result.payload && lr.result.payload.readable) msg = lr.result.payload.readable;
    }
    out.innerHTML = "<b>" + (msg.split("<")[0]) + "</b>";
    logLine("[rv] " + msg);
    if (/migrated/i.test(msg)) { out.innerHTML += "<br><span style='color:var(--ok)'>Old wallet is now revoked. Copy the new wallet address from the header — that's your identity key now.</span>"; }
  } catch (err) {
    const safeMsg = String(err?.message || err).replace(/</g, "&lt;");
    out.innerHTML = "<span style='color:var(--err)'>Recovery failed: " + safeMsg + "</span>";
    logLine("[rv] failed: " + safeMsg);
  } finally {
    $("rv-submit").disabled = false;
    $("rv-submit").textContent = "Recover →";
  }
};
...
function formError(msg, el) {
  const vs = el || $("verify-status");
  vs.innerHTML = `<b style="color:var(--err)">${msg}</b>`;
  vs.style.color = "var(--err)";
  setTimeout(() => { vs.style.color = ""; }, 8000);
}
$("submitBtn").onclick = async () => {
  const needTwo = !$("fStrict") || $("fStrict").checked;
  if (!walletAddr) { formError("Click \"Connect\" first."); return; }
  if (picked.length < (needTwo ? 2 : 1)) {
    formError(needTwo
      ? "Pick at least 2 sources (strict mode is on)."
      : "Pick at least 1 source.");
    return;
  }
  const name = $("f-name").value.trim();
  if (!name) { formError("Artist name is required."); return; }
  if (name.length > 64) { formError("Artist name must be 64 characters or fewer."); return; }
...
(async function boot() {
  try {
    ...
    // First visit with no wallet → prompt to connect (demo or real).
    // Returning visitors with a saved demo key go straight to the app.
    if (!walletAddr) {
      const savedPk = localStorage.getItem(LS_KEY);
      if (savedPk) {
        attachAccount(savedPk);
        $("localAcctBtn").textContent = shortAddr(walletAddr);
      } else {
        setTimeout(() => openWalletModal(), 500);
      }
    }
  } catch (e) {
